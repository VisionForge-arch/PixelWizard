...
def register_spatial_control(model, inject_blocks=None):
    ...
    def create_block_hook(block_idx):
        def forward_hook(module, args, output):
            ...
            x_in = output.to(target_dtype)
            ctx_in = lr_context.to(target_dtype)

            attn_delta = ca(x_in, ctx_in, context_lens, rope_infos=rope_infos)
            attn_delta = attn_delta.to(output.dtype)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        B, F_n, C, H, W = noisy_image_or_video.shape
        seq_len = F_n * (H // 2) * (W // 2)
