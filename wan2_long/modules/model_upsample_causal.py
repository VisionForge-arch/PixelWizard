...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        def cross_attn_ffn(x_, context_, context_lens_, e_, crossattn_cache=None):
            x_ = x_ + self.cross_attn(self.norm3(x_), context_, context_lens_, crossattn_cache=crossattn_cache)
            y_ = self.ffn(
                (self.norm2(x_).unflatten(dim=1, sizes=(num_frames, frame_seqlen)) * (1 + e_[4]) + e_[3]).flatten(
                    1, 2
                )
            )
            x_ = x_ + (y_.unflatten(dim=1, sizes=(num_frames, frame_seqlen)) * e_[5]).flatten(1, 2)
            return x_
    ...
    def _forward_inference(
        ...
        for block_index, block in enumerate(self.blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                raise AssertionError("Gradient checkpointing is not supported in causal inference mode.")
            kwargs.update(
                {
