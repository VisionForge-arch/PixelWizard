...
def rope_params(max_seq_len, dim, theta=10000):
    assert dim % 2 == 0      
    freqs = torch.outer(
        torch.arange(max_seq_len),     # [max_seq_len] = [1024]
        1.0 / torch.pow(theta,
                        torch.arange(0, dim, 2).to(torch.float64).div(dim)))  # [dim/2] 
    freqs = torch.polar(torch.ones_like(freqs), freqs)   # 转换为复数形式
    return freqs # [1024, dim//2]


# @amp.autocast(enabled=False)
def rope_apply(x, grid_sizes, freqs):
    # x: [B, seq_len, num_heads, head_dim] 例如：[1, 32760, 12, 128]
    # grid_sizes: [B, 3] 例如：[1, 3] 包含 (F=21, H=30, W=52)
    # freqs: [1024, head_dim//2] 例如：[1024, 64]
    
    n, c = x.size(2), x.size(3) // 2 # n=num_heads, c=head_dim//2

    # split freqs 分割频率为三个维度 # [1024, 22], [1024, 21], [1024, 21]
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        # precompute multipliers 将输入转换为复数
        x_i = torch.view_as_complex(x[i, :seq_len].to(torch.float64).reshape(
            seq_len, n, -1, 2))  # [seq_len, n, c, 2] -> [seq_len, n, c]复数
        
        # 为每个3D位置生成对应的频率
        freqs_i = torch.cat([
            freqs[0][:f].view(f, 1, 1, -1).expand(f, h, w, -1), # [21, 22] → [21, 1, 1, 22] → [21, 30, 52, 22]
            freqs[1][:h].view(1, h, 1, -1).expand(f, h, w, -1),  # [30, 21] → [1, 30, 1, 21] → [21, 30, 52, 21]
            freqs[2][:w].view(1, 1, w, -1).expand(f, h, w, -1)  # [52, 21] → [1, 1, 52, 21] → [21, 30, 52, 21]
        ],dim=-1).reshape(seq_len, 1, -1)  # [seq_len, 1, c]
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        e = (self.modulation + e).chunk(6, dim=1)    # list 6个 [1, 1, 1536]
        # assert e[0].dtype == torch.float32

        # self-attention
        y = self.self_attn(
            self.norm1(x) * (1 + e[1]) + e[0], seq_lens, grid_sizes,
            freqs)             # [1, 21*30*52, 1536]
        # with amp.autocast(dtype=torch.float32):
        x = x + y * e[2]         # [1, 21*30*52, 1536]

        # cross-attention & ffn function
        def cross_attn_ffn(x, context, context_lens, e):
            x = x + self.cross_attn(self.norm3(x), context, context_lens)  #[1, 21*30*52, 1536]
            y = self.ffn(self.norm2(x) * (1 + e[4]) + e[3])               # [1, 21*30*52, 1536]
            # with amp.autocast(dtype=torch.float32):
            x = x + y * e[5]                                             # [1, 21*30*52, 1536]      
            return x

        x = cross_attn_ffn(x, context, context_lens, e)  # [1, 21*30*52, 1536]
        return x
...
class Head(nn.Module):
    ...
    def __init__(self, dim, out_dim, patch_size, eps=1e-6):
        ...
        # modulation
        self.modulation = nn.Parameter(torch.randn(1, 2, dim) / dim**0.5) # [1, 2, 1536]
    ...
    def forward(self, x, e):
        ...
        # assert e.dtype == torch.float32
        # with amp.autocast(dtype=torch.float32):
        e = (self.modulation + e.unsqueeze(1)).chunk(2, dim=1) # list 两个[1, 1536]
        x = (self.head(self.norm(x) * (1 + e[1]) + e[0]))    # [1, 21*30*52, 64]
        return x
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def _forward(
        ...
        grid_sizes = torch.stack(
            [torch.tensor(u.shape[2:], dtype=torch.long) for u in x])  # 后面的size
        x = [u.flatten(2).transpose(1, 2) for u in x]                  # [1, 21*30*52, 1536]
        seq_lens = torch.tensor([u.size(1) for u in x], dtype=torch.long)  # 21*30*52=32760
        ...
        # time embeddings
        # with amp.autocast(dtype=torch.float32):
        e = self.time_embedding(sinusoidal_embedding_1d(self.freq_dim, t).type_as(x)) # [1, 1536]
        e0 = self.time_projection(e).unflatten(1, (6, self.dim))                      # [1, 6, 1536]
        ...
        for ii, block in enumerate(self.blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                x = block(x, **kwargs)      # [1, 21*30*52, 1536]
        ...
        x = self.head(x, e)       # [1, 21*30*52, 64]

        # unpatchify
        x = self.unpatchify(x, grid_sizes)  # [1, 64, 21, 30, 52]
