...
class WanTI2V:
    ...
    def __init__(
        ...
        if wan_ckpt is not None:
            logging.info(f"Loading checkpoint from {wan_ckpt}")
            state_dict = torch.load(wan_ckpt, map_location="cpu")

            # Extract generator weights from training checkpoint
            ...
            for k, v in generator_state_dict.items():
                ...
                cleaned[k] = v
            generator_state_dict = cleaned

            # Strip "model." prefix if present
            if all(k.startswith("model.") for k in generator_state_dict):
                generator_state_dict = {
                    k[len("model."):]: v for k, v in generator_state_dict.items()
                }

            missing, unexpected = self.model.load_state_dict(
                generator_state_dict, strict=False)
            logging.info(
                f"Loaded checkpoint: {len(missing)} missing, "
